package com.example.parking.presentation.fragments.parkingSpot.create
...
import com.example.parking.presentation.fragments.parkingSpot.create.elm.State
import com.example.parking.presentation.fragments.parkingSpot.create.elm.Event
import com.example.parking.presentation.fragments.parkingSpot.create.elm.Effect
import com.example.parking.presentation.fragments.parkingSpot.create.elm.storeFactory
import vivid.money.elmslie.android.base.ElmFragment


class CreateSpotsFragment : ElmFragment<Event, Effect, State>() {
    ...
    override fun onCreateView(
        ...
        btContinue?.setOnClickListener {
            if (!isFieldEmpty(etNumSpot)) {
                val parkingSpot: ParkingSpot = ParkingSpot(parkingNumber = etNumSpot.text.toString().toInt())
                store.accept(Event.Ui.CreateClick(parkingSpot))
            }
        }
    ...
    override fun handleEffect(effect: Effect) = when (effect) {
        ...
        is Effect.ShowErrorCreateParkingSpot -> Toast.makeText(
            activity,
            "Unable to create a parkingSpot, error on the server! Please check the entered data!",
            Toast.LENGTH_SHORT
        ).show()
    ...
    private fun toParkingSpotsFragment() {
    ...
    private fun showConfirmDialog(parkingSpot: ParkingSpot) {
        ...
        textOutput.text = "Parking spot number: " + parkingSpot.parkingNumber.toString()
